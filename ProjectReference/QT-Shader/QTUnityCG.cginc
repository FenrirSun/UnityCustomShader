...
inline half3 DecodeLightmap( fixed4 color )
{
	...
	fixed Le = color.z * 255 + color.w;
	fixed3 Xp_Y_XYZp;
	Xp_Y_XYZp.y = exp2((Le - 127) / 2);
	Xp_Y_XYZp.z = Xp_Y_XYZp.y / color.y;
	Xp_Y_XYZp.x = color.x * Xp_Y_XYZp.z;
	fixed3 vRGB = mul(Xp_Y_XYZp, InverseM);
	return max(vRGB, 0);*/

	float p = color.a;
	color.rgb = color.rgb * (pow(2, p) - 1) * 10;
	//color.rgb = color.rgb * (exp(p) - 1) * 7;
	color.a = 1;
	return color;
