...
//*#ifqtdef editor
	...
	void surf (Input IN, inout SurfaceOutput o) {
		...
		fixed3 lay0B = QTUnpackNormal(tex2D(_QTBumpMap, nuv0).ba);

		fixed2 nuv1 = frac(IN.uv_Splat0) * 0.5;
		nuv1.x += 0.5;
		fixed3 lay1B = QTUnpackNormal(tex2D(_QTBumpMap, nuv1).ba);

		fixed2 nuv2 = frac(IN.uv_Splat0) * 0.5;
		nuv2.y += 0.5;
		fixed3 lay2B = QTUnpackNormal(tex2D(_QTBumpMap, nuv2).ba);

		fixed2 nuv3 = frac(IN.uv_Splat0) * 0.5;
		nuv3 += 0.5;
		fixed3 lay3B = QTUnpackNormal(tex2D(_QTBumpMap, nuv3).ba);
		//细节法线合并
		...
		//解压法线
		fixed3 n = QTUnpackNormal(tranSpaceNormal.rg);
		//发光系数
