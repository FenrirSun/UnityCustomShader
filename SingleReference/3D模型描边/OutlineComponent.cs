...
public class OutlineComponent : MonoBehaviour
{
    ...
    /// 描边shader，做成静态避免重复加载
    /// </summary>
    private static Shader outlineMaskShader;
    /// <summary>
    /// 描边shader，做成静态避免重复加载
    /// </summary>
    private static Shader outlineFillShader;
    ...
    void Awake()
    {
        ...
        LoadShader("Shader/Effect/OutlineMask", (shader ) => {
            outlineMaskShader = shader;
        }).Then(() => LoadShader("Shader/Effect/OutlineFill", (shader) => {
            outlineFillShader = shader;
        })).Then(PrepareMaterial);
    }

    private IPromise LoadShader(string _path, Action<Shader> _cb)
    {
        Promise promise = new Promise();

#if UNITY_EDITOR

        var aao = Resources.LoadAsync(_path);

        _cb?.Invoke(aao.asset as Shader);
        promise.Resolve();

#else
        AssetsManager.Instance.LoadShader(_path).AddCallback(OnLoadComplete, OnError);

        //加载完毕
        void OnLoadComplete(AssetsAsyncOperation _operation)
        {
            _cb?.Invoke(_operation.GetAsset<Shader>());
            promise.Resolve();
        }
